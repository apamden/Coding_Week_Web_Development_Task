...
const products=[{
    ...
    "description": "Dry Rose FUpper Powder Gulab Powder 50 Gram • Treats Wounds",
    "price": 70,
    ...
    const showProducts =(products)=>{
        const htmlString= products.map((product)=>{
            ...
            <img src="${product.thumbnail}" class="pic"></img>
            </div>`;
        })
        .join('');
    ...
    const filter = products.filter((product) => {
        return (
            product.title.toUpperCase().includes(searchStr) ||product.description.toUpperCase().includes(searchStr)
        );
    });
    showProducts(filter);
});
